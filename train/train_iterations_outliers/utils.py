...
class TabNetClassifierWrapper:
    """Wrapper para TabNetClassifier para compatibilidad con sklearn pipeline."""
    def __init__(self, n_d=8, n_a=8, n_steps=3, gamma=1.3, n_independent=2,
                 n_shared=2, lambda_sparse=1e-3, optimizer_fn=None,
                 optimizer_params=dict(lr=2e-2), scheduler_params=None,
                 scheduler_fn=None, mask_type='sparsemax', seed=42,
                 verbose=0, device_name='auto'):
        if not TABNET_AVAILABLE:
            raise ImportError("TabNet is not available. Install with: pip install pytorch-tabnet")
...
class SwiFTClassifier:
    """
    Sparse Weighted Fusion Transformer (SwiFT) para datos tabulares.
    Implementación simplificada basada en transformer con atención sparse.
    """
    def __init__(self, d_model=64, nhead=4, num_layers=2, dim_feedforward=128,
                 dropout=0.1, max_epochs=50, batch_size=64, lr=1e-3, seed=42):
...
class SwiFTModel(nn.Module):
    """Modelo interno de SwiFT basado en Transformer."""
    def __init__(self, n_features, d_model, nhead, num_layers,
                 dim_feedforward, dropout, n_classes):
    ...
    def forward(self, x):
        ...
        # Capa de salida
        x = self.output_layer(x)  # (batch_size, n_classes)
...
class TTLClassifier:
    """
    Transformer-based Tabular Learning (TTL) para clasificación.
    Implementación de transformer optimizado para datos tabulares.
    """
    def __init__(self, d_model=128, nhead=8, num_layers=3, dim_feedforward=256,
                 dropout=0.15, max_epochs=100, batch_size=32, lr=1e-3, seed=42):
        if not TORCH_AVAILABLE:
            raise ImportError("PyTorch is not available. Install with: pip install torch")
...
class TTLModel(nn.Module):
    """Modelo interno de TTL basado en Transformer con LayerNorm mejorado."""
    def __init__(self, n_features, d_model, nhead, num_layers,
                 dim_feedforward, dropout, n_classes):
    ...
    def forward(self, x):
        ...
        # Proyección y normalización de entrada
        x = self.input_projection(x)  # (batch_size, 1, d_model)
        x = self.input_norm(x)
...
def initialize_classification_results():
    ...
    results["f1_score_macro"] = {"train": [], "test": []}
    return results
...
def calculate_classification_metrics(y_true, y_pred, average='weighted'):
    ...
    Returns:
        tuple: (accuracy, precision, recall, f1_score, f1_score_macro)
    """
    ...
    return acc, precision, recall, f1, f1_macro

def accumulate_classification_results(results, tipo="train", acc=None, precision=None, recall=None, f1=None, f1_macro=None):
    ...
    results["f1_score_macro"][tipo].append(f1_macro)
...
def train_test_class_nested(df_imputed, n_clases, model_name, model_config, element = "Nitrogen",
    ...
    for fold_idx, (train_index, val_index) in enumerate(skf_outer.split(X, y)):
        ...
        avg = "micro"
        test_acc, test_prec, test_rec, test_f1, test_f1_macro = calculate_classification_metrics(
        ...
        accumulate_classification_results(
            results, "test", test_acc, test_prec, test_rec, test_f1, test_f1_macro
        )
    ...
    if usar_smote:
        final_pipeline = ImbPipeline([
            ('scaler', StandardScaler()),
            ('smote', SMOTE(random_state=seed)), # TODO: Ver si es correcto usar SMOTE aqui
    ...
    resultados_finales = {
        ...
        'f1_macro_test': np.mean(results['f1_score_macro']['test']),
...
def build_pipeline(model_config, usar_smote = False, seed=42):
    if usar_smote:
        pipeline_steps = [
            ('scaler', StandardScaler()),
            ('smote', SMOTE(random_state=seed)),
...
def train_test_model(df_imputed, n_clases, model_name, model_config, element = "Quartiles",
                              usar_smote=True, mostrar_graficos=True, calcular_shap=True,
                              h5_file=None,
                              dir_path= "../",
...
def train_test_model_all_predictions(df_imputed, n_clases, model_name, model_config, element = "Nitrogen",
    ...
    X, y, feature_names, class_dist = extract_X_y_classification(
        df_imputed, n_clases=n_clases, element=element, CFG=CFG, best_variables=best_variables
    )
    ...
    for fold_idx, (train_index, val_index) in enumerate(skf_outer.split(X, y)):
        ...
        fold_acc, fold_prec, fold_rec, fold_f1, fold_f1_macro = calculate_classification_metrics(
    ...
    avg = "weighted" if n_clases is not None else "micro"
    global_acc, global_prec, global_rec, global_f1, global_f1_macro = calculate_classification_metrics(
    ...
    if calcular_shap and best_model is not None:
        ...
        X_train, X_test, y_train, y_test, feature_names, class_dist = preparar_datos(
            df_imputed, n_clases=n_clases, element=element, best_variables=best_variables, CFG=CFG, random_state=seed
        )
...
def build_transformer_pipeline(model_config, usar_smote=False):
    """
    Construye un pipeline para modelos transformer.
    Los transformers manejan sus propios escaladores internos.
    """
    if usar_smote:
        pipe = ImbPipeline([
...
def train_test_transformers(df_imputed, n_clases, model_name, model_config, element="Nitrogen",
                            usar_smote=False, mostrar_graficos=True, calcular_shap=True,
                            dir_path="../",
                            best_variables=None, CFG=None):
    """
    Función para entrenar y evaluar modelos basados en transformers.
    Similar a train_test_model pero adaptada para arquitecturas transformer.
    
    Args:
        df_imputed (DataFrame): DataFrame con datos imputados.
        n_clases (int): Número de clases para la codificación.
        model_name (str): Nombre del modelo (ej: 'SwiFT', 'TTL', 'TabNet').
        model_config (dict): Configuración del modelo (estimator y param_grid).
        element (str): Elemento a utilizar para la codificación individual.
        usar_smote (bool): Si se debe usar SMOTE para balancear clases.
        mostrar_graficos (bool): Si se deben mostrar gráficos de confusión.
        calcular_shap (bool): Si se deben calcular valores SHAP.
        dir_path (str): Ruta para almacenar los modelos.
        best_variables (list): Lista opcional de variables mas importantes.
        CFG: Configuration object.
    ...
    # Preparar datos (sin escalado, los transformers lo manejan internamente)
    X_train, X_test, y_train, y_test, feature_names, class_dist = preparar_datos(
        df_imputed, n_clases, element=element, best_variables=best_variables, CFG=CFG
    )

    # Construir pipeline específico para transformers
    pipe = build_transformer_pipeline(model_config=model_config, usar_smote=usar_smote)
    ...
    grid = GridSearchCV(
        ...
        cv=cv,
        scoring='f1_micro', 
        n_jobs=N_THREADS,  # Transformers suelen usar GPU/CPU internamente
        verbose=2,
        return_train_score=True
    )
    grid.fit(X_train, y_train)
    ...
    # Métricas train del mejor modelo (usando cross-validation)
    print(f"Evaluating best model with cross-validation...")
    nested_score = cross_validate(
    ...
    # Métricas test
    acc_test, f1_test, f1_test_macro = return_classification_metrics(y_test, y_test_pred)

    # Classification Report
    class_report = classification_report(y_test, y_test_pred)
    os.makedirs(f"{dir_path}/results", exist_ok=True)
    
    report_filename = f"{dir_path}/results/{model_name.replace(' ', '_')}_classification_report.txt"
    with open(report_filename, "w") as f:
        f.write(class_report)
    print(class_report)
    class_report_dict = classification_report(y_test, y_test_pred, output_dict=True)
    ...
    print_classification_report(
        model_name, n_clases, acc_train, acc_test, 
        f1_train, f1_test, grid.best_params_, class_dist
    )
    ...
    fig_cm_test = plot_confusion_matrix(
        ...
        title=f"{model_name} Test - {n_clases if n_clases else element} classes"
    )
    if mostrar_graficos:
        plt.show()

    # SHAP values (usando KernelExplainer para transformers)
    shap_values = None
    X_scaled_df = None
    ...
    if calcular_shap:
        try:
            ...
            # Para transformers usamos KernelExplainer
            ...
            # Crear DataFrame para SHAP
            X_test_df = pd.DataFrame(X_test, columns=feature_names)
            
            # Usar muestra reducida para SHAP (transformers son más lentos)
            sample_size = min(100, len(X_test_df))
            background = shap.sample(X_test_df, sample_size, random_state=42)
            
            # KernelExplainer para modelos transformer
            explainer = shap.KernelExplainer(clf.predict_proba, background)
            shap_values_raw = explainer.shap_values(background, nsamples=50)
            ...
            if CFG.individual_train:
                title_suffix = f"{element}"
            elif CFG.cuartiles_train:
                title_suffix = "Quartiles"
            else:
                title_suffix = f"{n_clases} classes"
            
            plt.title(f"SHAP Feature Importance - {model_name} ({title_suffix})", 
                     fontsize=14, pad=20)
            plt.xlabel("Mean |SHAP value|", fontsize=12)
            plt.tight_layout()
            plt.savefig(f"{path_images_shap}shap_bar.png", dpi=300, bbox_inches='tight')
            ...
            X_scaled_df = background
            
            print(f"SHAP values calculated and saved successfully.")
            
        except Exception as e:
            print(f"Warning: Could not calculate SHAP values for {model_name}: {str(e)}")
            shap_values = None
            X_scaled_df = None
...
def compare_classification_models(resultados_dict, CFG):
    ...
    plot_comparative_bars(resultados_dict, 'f1_macro_test')
    # Tabla resumen
    ...
    for model_name, resultados in resultados_dict.items():
        ...
        print(f"  F1 Micro: {best_result['f1_test']:.4f}")
        print(f"  F1 Macro: {best_result['f1_macro_test']:.4f}")
    ...
    for model_name, resultados in all_results.items():
        for resultado in resultados:
            resultados_completos.append({
                ...
                'F1_Macro_Test': resultado['f1_macro_test'],
...
def permutation_importance_all_elements(class_results, df_imputed, dir_path_permutation, list_elements, CFG, all_models = False, best_variables = None, random_state = 42):
    ...
    for i in range(len(list_elements)):
        ...
        for alg in algorithms:
            X_train, X_test, y_train, y_test, feature_names, class_dist = preparar_datos(
                df_imputed, n_clases=3, element=list_elements[i], CFG=CFG, best_variables=best_variables, random_state=random_state
            )
            if all_models:
...
def most_frequent_variables_analysis_PERMUTATION(df_imputed, dict_perm_impt_vals, dir_path_permutation, CFG, best_variables = None, random_state=42):
    percentages = [0.7, 0.8]
    for percentage in percentages:
        for element in dict_perm_impt_vals: #recorre cada modelo (elementos o cuartil)
            ...
            for alg in dict_perm_impt_vals[element]:
                ...
                X_train, X_test, y_train, y_test, feature_names, class_dist = preparar_datos(
                df_imputed, n_clases=3, element=element, CFG=CFG, best_variables=best_variables, random_state=random_state
                )
...
def permutation_importance_NPK(all_results, df_imputed, dir_path_permutation, class_path, CFG, all_models = False, random_state = 42):
    ...
    most_frequent_variables_analysis_PERMUTATION(df_imputed, dict_perm_impt_vals, dir_path_permutation, CFG=CFG, random_state = random_state)
...
def permutation_importance_Quartiles(all_results, df_imputed, dir_path_permutation, class_path, CFG, all_models = False, best_variables = None, random_state = 42):
    
    os.makedirs(dir_path_permutation, exist_ok=True)
    dict_perm_impt_vals = permutation_importance_all_elements(all_results, df_imputed, dir_path_permutation,
                                                              list_elements = ['Quartiles'], CFG=CFG, all_models=all_models, best_variables=best_variables, random_state=random_state)
    most_frequent_variables_analysis_PERMUTATION(df_imputed, dict_perm_impt_vals, dir_path_permutation, CFG=CFG, best_variables=best_variables, random_state=random_state)
