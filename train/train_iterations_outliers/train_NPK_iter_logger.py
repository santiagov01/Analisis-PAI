...
# N_JOBS_MODELS: cuántas tareas (modelo, elemento) se entrenan en paralelo (paralelismo "externo").
...
OUTLIER_PERCENTAGES = [0.2, 0.3, 0.4]
...
LOG_FILE = f"{LOG_DIR}outliers_experiment_npk.log"

logger = logging.getLogger("outliers_experiment_npk")
...
# cada outlier_percentage). Sin el lock, dos procesos escribiendo a la vez
# pueden pisarse la escritura o dejar el JSON corrupto/incompleto.
...
def train_test_model_outliers(df_imputed, n_clases, model_name, model_config, element="Quartiles",
    ...
    logger.info(f"[{model_name} | {element}] Iniciando entrenamiento (seed={seed}, outlier={outlier}).")
    ...
    logger.info(f"[{model_name} | {element}] GridSearch finalizado. Mejores parámetros: {grid.best_params_}")
    ...
    logger.info(f"[{model_name} | {element}] Classification report (test):\n{class_report}")
    ...
    if fig_cm_test is not None:
        ...
        try:
            ...
            logger.info(f"[{model_name} | {element}] Matriz de confusión (test) guardada en: {cm_fig_path}")
        except Exception:
            logger.exception(f"[{model_name} | {element}] No se pudo guardar la matriz de confusión en disco.")
        finally:
    ...
    logger.info(f"[{model_name} | {element}] Entrenamiento completo. acc_test={acc_test:.4f} | f1_test={f1_test:.4f}")
...
def _entrenar_todas_las_tareas(models_config, df_imputed, class_path, seed, outlier, n_jobs):
    """Corre entrenar_modelo_por_elemento para todas las combinaciones (modelo, elemento)
    con el n_jobs indicado."""
...
def run_non_nested_iteration(df_imputed, models_config, class_path, seed, outlier):
    ...
    try:
        all_results_list = _entrenar_todas_las_tareas(
    ...
    logger.info(f"[NO NESTED] Total de combinaciones: {len(all_results_list)}")
...
def build_nested_iterations_summary(base_path, n_iterations, base_seed=42, output_name="resumen_metricas_nested_iteraciones.csv",
    ...
    logger.info(f"Resumen de iteraciones (nested) guardado en: {output_path}")
